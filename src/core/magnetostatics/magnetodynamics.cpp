...
void System::System::integrate_magnetodynamics(
    [[maybe_unused]] bool initial_step) {
...
#endif // field-response models

#ifdef ESPRESSO_THERMAL_STONER_WOHLFARTH
  /* Stochastic/thermal model. It advances a kinetic process by one time step
   * and therefore must not run during the pre-loop force evaluation.
   */
  if (not initial_step) {
